...
struct segtree{
    ...
    void init(int n){ // to initialize the size of the array
        size = 1;
        while(size < n) size *= 2;
        values.resize(2 * size);
    }

    void build(vector < ll > &a , int x , int lx , int rx){ // to build the tree
        if(rx - lx == 1){
    ...
    void set(int i , int v , int x , int lx , int rx){ // to set a value in the tree
        if(rx - lx == 1){
            values[x] = single(v);
            return;
        }
    ...
    item calc(int l , int r , int x , int lx , int rx){ // to calculate the value of the range
    ...
    int find(int k , int x , int lx , int rx){ // to find the kth element in the tree
    ...
    int first_above(int v , int l , int x , int lx , int rx){ // to find the first element above a value in a range
